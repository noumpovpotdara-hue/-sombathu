...
<style>
  /* Body និង background image */
  body {
    margin: 0; padding: 0;
    font-family: 'Khmer OS System', Arial, sans-serif;
    background-image: url('https://raw.githubusercontent.com/noumpovpotdara-hue/sombathu/main/Picture.txt/image.jpg');
...
divs.forEach(btnDiv => {
  ...
  btn.addEventListener('click', () => {
    ...
    detailsList.innerHTML = detailsData[key].map(item => `<li>${item}</li>`).join('');
